...
def recommend_scenario(
    ...
    feasible = {
        s: r for s, r in lev_results.items()
        if r.get("feasibility", True)
    }
    if not feasible:
        return None
...
def get_recommendation_reasons(
    ...
    *,
    ...
    if uplift > 0 and recommended != "즉시":
        reasons.append(
            f"즉시벌채 대비 NPV +{int(uplift/1e4):,}만원/ha 증가 "
            f"(등급 상승 + 추가 성장)"
        )
    ...
    if carbon > 0:
        reasons.append(
            f"탄소수익 {int(carbon/1e4):,}만원 포함 (KOC > WTA hurdle 17,039원 충족)"
        )
    ...
    if legal_min_age > 0 and T < legal_min_age:
        reasons.append(
            f"⚠️ T={T}년 < 법정 {legal_min_age}년 — 법적 예외 사유 없으면 벌채 불가"
        )
    elif legal_min_age > 0 and recommended == "연장KOC":
        reasons.append(
            f"법정 {legal_min_age}년 + 연장으로 산림탄소상쇄 사업 신청 가능"
        )
...
def get_next_actions(
    ...
    *,
    ...
    if recommended == "연장KOC":
        ...
        actions.append(
            "📋 신청 시기: 매년 1-3월. 시·군·구청 산림과 별도 안내문 확인."
        )
        ...
        actions.append(
            "📞 충북도 산림과 (043-220-XXXX) — 벌채허가 신청 단계 안내 확인"
        )
...
def generate_kakao_message(
    ...
    *,
    ...
    base = {
        ...
        "연장KOC": (
            f"우리 산 산림탄소상쇄 신청 추천 (NPV {npv_label}). "
            "벌채 안 하고 매년 KOC 받는 사업."
        ),
        "간벌+10년": (
        ...
        "임산물": (
            f"우리 산 표고 재배 병행 추천 (NPV {npv_label}). "
            "충북농기원 컨설팅 + 산림청 보조 신청."
        ),
    }
...
if __name__ == "__main__":
    ...
    fake = {
        "즉시": {"npv": 50_000_000, "npv_q05": 40_000_000, "npv_q95": 60_000_000,
                 "feasibility": True, "T_optimal": 50, "carbon_revenue": 0},
        "10년": {"npv": 65_000_000, "npv_q05": 50_000_000, "npv_q95": 80_000_000,
                 "feasibility": True, "T_optimal": 60, "carbon_revenue": 0},
        "연장KOC": {"npv": 75_000_000, "npv_q05": 55_000_000, "npv_q95": 95_000_000,
                    "feasibility": True, "T_optimal": 60, "carbon_revenue": 5_000_000,
                    "kau_breakeven_warning": "KAU 17,200 → 16,300원 이하 시 LEV 음수"},
        "간벌+10년": {"npv": 80_000_000, "npv_q05": 65_000_000, "npv_q95": 95_000_000,
                      "feasibility": True, "T_optimal": 60, "subsidy_revenue": 4_400_000},
    }
    ...
    print("\n[검증 4] reasons")
    rs = get_recommendation_reasons("간벌+10년", fake, "균형",
