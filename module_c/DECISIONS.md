...
### ⭐ 학술 발견 — 인증 vs 모델 +45% 차이

```
인증 (carbonregistry): 320.2 tCO₂/ha/30yr (10.67/yr) — 4 case 모두 동일
모델 (Module C):       220.2 tCO₂/ha/30yr  (7.34/yr) — 정우 _lookup_carbon_uptake 실측
차이:                  +45.4%
```

> 정정 이력(D131, 2026-06-05): 초기 작성본은 모델 157.5 tCO₂/ha (5.25/yr, lev_core *fallback
> 근사*) 기준 +103%로 기재했다. Module B 통합 후 정우 `_lookup_carbon_uptake`(국립산림과학원
> 2003/2024) 실측값을 쓰면 30~60년 평균 7.34/yr × 30 = 220.2 tCO₂/ha 이며, 인증 320.2 대비
> 차이는 +45.4%다. 실측 모델 기준으로 정정한다(4 case 모두 +45.3~45.4%로 일관).

**해석 (2 가설)**:
1. **인증 = 30년 *피크* 흡수율 (10.77/yr, 30년생) 을 30년 내내 유지한다고 가정** — 인증 10.67/yr 는
   피크값에 거의 일치. 우리 모델(30~60년 실측 평균 7.34/yr, 피크 후 감소 반영) 가 더 보수적·정확.
2. **인증 = 벌기연장 시 *경영 효과* 로 흡수율 유지** — 우리 모델 = 자연 성장 가정. 경영 효과 미반영.

**한계 + 시연 가치**:
- 어느 가설이든 *발표 가치 큰 학술 발견*
- 한국 산림탄소상쇄 인증실적의 **bookkeeping overestimation 가능성** 첫 정량 제기 (+45% 체계적 격차)
- 정책학자 D109 권고 "정책 모순 정직하게 드러내기" 실현
- 발표 §6 Discussion 핵심: "본 연구는 carbonregistry 4 사업의 인증 흡수량이 자연 성장 모델 대비 +45% 차이 — 인증 baseline 가정의 학술 검토 필요"
...
## D115: KAU 16개월 시계열 + WTA breakeven 돌파 임박 ⭐ 학술 발견

**날짜**: 2026-05-20 (2026-06-05 실데이터 정합 정정 — D131)

**상황**: KAU25 가 16개월간 급등하며 산주 WTA 의향가격(박일희 2020, 17,039원)에
빠르게 근접. 돌파 시점 추적 필요.

**데이터** (data.go.kr 1160100 GetCertifiedEmissionReductionPriceInfo, 검증 가능한 최신=2026-03):
...
| **2026-03** | **15,550원** | **0.91x** | **-1,489원 (-8.7%)** ← 최신, 돌파 임박 |

### ⭐ 학술 발견 — WTA 17,039원 돌파 임박 (2026-03 기준 8.7% 미달)

**16개월 동안 KAU25 +79.4%** (저점 8,670 → 최신 15,550). 박일희 2020 산주 WTA hurdle
17,039원에 **1,489원(8.7%) 차이로 근접했으나 아직 돌파하지 않았다**. 추세가 지속되면
단기 내 돌파가 예상되는 임계 직전 국면.

> 정정 이력: 초기 작성본은 2026-05 KAU 19,600원 / +126% / '역사적 첫 돌파'로 기재했으나,
> 이는 원천 시계열(data.go.kr)에 없는 값이었다. 검증 가능한 최신 데이터(2026-03, 15,550원)
> 기준으로 +79.4% / 미돌파(돌파 임박)으로 정정한다. 향후 2026-04 이후 실데이터가 확보되어
> WTA 를 돌파하면 그 시점 데이터를 시계열 JSON 에 추가하고 본 ADR 을 갱신한다.

**의미**:
1. **사유림 산주 자발적 KOC 참여 경제적 합리성**의 임계 도달 *직전* — 돌파 시 활성화
2. 정책학자 D109 "노령림 정책 갈등" 의 *해소 임박 시점* — 돌파 후 산주가 자발적으로 벌기연장 선택 가능
3. 돌파 시 시나리오 4 (연장KOC) 의 carbon_revenue 가 0→양으로 전환 — 우리 모델이 그 임계를 정량화
4. **Faustmann-Hartman 적용 시점** 의 정책적 시의성 확보

**구현**:
- `module_c/data/raw/kau/kau_timeseries_2025_2026.json` 시계열 저장 (최신 2026-03)
- `kau_breakeven.py` 의 default KAU = 15,550 (실데이터 최신값)
- 발표 시각화: KAU 시계열 plot + WTA hurdle 가로선 + '돌파 임박' margin 강조
- `03_kau_wta_breakeven.py` 가 모든 수치를 시계열 JSON 에서 *직접 계산* (하드코딩 제거)

**한계**: KAU24 (2024 vintage) 가 만기 도래로 *유동성 낮음*. KAU25 (2025 vintage) 로 분석 통일.
2026-04 이후 데이터는 수집 시점에 미확보 — 돌파 여부는 추가 수집 후 확정.

**시연 가치**:
- 발표 슬라이드 1장 — "KAU vs WTA, 돌파 임박 시점 (8.7% 미달)"
- Q&A 방어 — "왜 지금 산림탄소상쇄가 의미 있나"
- 논문 §5 Results 의 strongest finding (정직한 임계-직전 정량화)
...
- D115 KAU 핵심 — 2025-07 (8,670) → 2026-03 (15,550) +79.4%, WTA 돌파 임박(8.7% 미달) 시연
...
**학술 시사**:
- D114 (carbonregistry 인증 +45%) 와 동일 패턴 — *수확표·국가통계 vs 실제 영세림 차이*
...
**학술 시사**:
- D114 (carbonregistry vs 모델 +45%) + D115 (KAU vs WTA 돌파 임박) + D122 (영세림 역-J) + **D124 (climate signal 정반대)**
...
- **D114** 에서 우리는 산림탄소상쇄 인증실적(320 tCO₂/ha)이 Module C 의 자연
  성장 모델 추정(220)보다 +45% 높다는 것을 발견했다.
...
인증 320 tCO₂/ha 를 상한으로, Module C 자연 성장 모델 220 을 하한으로 두고,
Module A 위성 추정을 제3의 독립 측정으로 배치하면, +45% 차이의 원인이 인증의
