...
작은 표 3개 (해송 p.30, 삼나무 p.56, 이태리포플러 p.102) pdfplumber 재추출.

⚠️ DRAFT 데이터 — 검증 미완료
================================================================
이 스크립트가 생성하는 데이터는 *모듈 B에서 신뢰성 검증 미완료*.

발견된 문제 (2026-05-13):
- 작은 표는 2 PDF 페이지로 분할 (예: 해송 = p.30 DBH 6-20 + p.31 DBH 22-40)
- 현재 코드는 p.30 만 읽음 → 큰 DBH 데이터 누락 가능
- 일부 행에서 *잘못된 위치에 값 매칭* 발견
  예: 해송 DBH 6cm × 수고 22m = 0.3652 m³ (물리적으로 불가능한 값)
- 원인 추정: 페이지 31 일부 텍스트가 페이지 30 텍스트에 섞여 들어옴

영향 평가:
- 충북 보은 (파일럿 지역) 주력 수종: 강원지방소나무, 잣나무, 낙엽송
- 작은 표 수종 (해송, 삼나무, 이태리포플러) 은 충북에서 *거의 안 쓰임*
- 따라서 *공모전 시연·발표에 영향 작음*

추후 (Day 2+):
- 페이지 30 + 31 결합 로직 필요 (큰 표의 둘째 페이지 파서와 비슷한 패턴)
- 또는 pdfplumber bbox 좌표로 페이지 정확히 분리

사용 주의:
- 이 파일이 만든 CSV/parquet 의 `품질` 은 *DRAFT* 로 표시됨
- 모듈 C·E에서 사용 시 `품질 == "OK"` 필터링 필수
- `growth_predict()` 에서 작은 표 수종 호출 시 사용자에게 경고 반환 권장

작은 표 특징:
- 흉고직경 6-20cm (8개)
- 수고 4-30m (27개)
- 좌상단 (작은 DBH, 작은 수고) 부터 채워짐
- 우상단·우하단은 물리적으로 빈 셀이 많음
"""
