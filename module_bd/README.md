...
**최종 업데이트:** 2026-05-15 (Day 3 마감)

**Repo 상태:** 35 commits + 7 DECISIONS + 5/5 진짜 PDF 데이터 + RAG 281 청크
...
### 1. `growth_predict()` — 임분 성장 + 탄소 흡수 ⭐ Day 3 확장
...
#     carbon_uptake_rate ⭐ NEW (tCO2/ha/yr, 국립산림과학원 표준)
```

### 2. `market_snapshot()` — 시장 종합 (D1 확장)
...
### 3. `cost_function()` — 5/5 진짜 데이터 ⭐ Day 3 정밀화
...
```python
rule = rotation_age("강원지방소나무", "사유림")  # → 40 (년)
```
...
│   ├── carbon_offset_chunk.py   # RAG 코퍼스 chunking ⭐ Day 3
│   └── standard_cost_diagnose{1-4}.py  # 표준품셈 진단 학습 기록
└── data/
    ├── raw/
    │   ├── carbon/              # 국립산림과학원 탄소흡수량 ⭐ Day 3
    │   │   └── carbon_uptake_2003.json
    │   ├── carbon_offset/       # 산림청 산림탄소상쇄 PDF 21개 ⭐ Day 3
    │   │   ├── 운영지침, 산림조사, 통합가이드북
    │   │   ├── 2025.1.2. 기준 (8 사업유형)
    │   │   ├── 2020.3.20. 기준 (7 사업유형)
    │   │   └── 타당성 평가 편람 (HWP 5개)
    ...
    │   ├── law_extracts/        # 법령 캐시
    │   ├── seedling/            # 산림청 묘목 단가 2025 ⭐ Day 3
    │   │   └── seedling_prices_2025.json
    ...
    │   └── carbon_chunks.jsonl          # ⭐ Day 3 NEW (281 청크)
    └── processed/
...
| 탄소흡수량 (수종·임령별) | 국립산림과학원 (3,212 표본 × 40년) | ⭐ Day 3 |
...
| 묘목 단가 | 산림청 2025 공식 (시행령 제16조) | 422원/본 (강원소나무) ⭐ Day 3 |
...
⏳ 산악기상 시계열 (가이드 §2.3) — 다음 우선순위
⏳ 등급분포 Weibull (NFI 활용) — 모듈 A 협업 필요
⏳ 임가경제·임산물 (작은 항목)
```

→ **9/11 완성 (82%)**. 핵심 책임 모두 완성.
...
✅ Person 4 (Module E) — chunked PDF (carbon_chunks.jsonl) ⭐ Day 3
...
3. ✅ 8 통합 데이터 자산 + 281 RAG 청크
4. ✅ 7 학술 결정 (DECISIONS.md)
...
### Day 4+ 우선순위
1. **산악기상 시계열** (가이드 §2.3, 2-3시간) — 모듈 B climate_scenario 진짜 보정
2. **임가경제·임산물** (1-2시간) — Faustmann 보조 수입
3. **등급분포 Weibull** (3-4시간) — 모듈 A 협업 시점에
4. **모듈 A 위성 GEE 시작** (1주+) — 큰 작업, 별도 세션
5. **팀원 collaborator 추가** — 현재 단계 결정 가능
...
# 4. 법정 벌기령 확인
rotation = rotation_age("강원지방소나무", "사유림")  # 40년
