...
### 완료 ✅ (Day 2 마감)
1. ✅ GitHub 원격 repo + Private 설정
2. ✅ KAU/KOC 를 market_snapshot() 에 통합
3. ✅ 표준품셈 PDF 진단 (4단계)
4. ✅ KOFPI 거리별 운반비 추출 (정우 발견 ⭐)
5. ✅ 노임 데이터 (대한건설협회) 보관
6. ✅ cost_function() 완성 — 5/5 진짜 데이터
7. ✅ shared/schemas.py — 팀 인터페이스 (Pydantic P2)
8. ✅ DECISIONS.md D1, D2, D3, D4 학술 자산화

### Day 3+ 우선순위
1. **산림탄소상쇄 RAG 코퍼스** (모듈 D + E, 2-3시간)
2. **묘목 단가 정밀화** — 산림용 종자·묘목 고시 (1시간)
3. **산악기상 시계열** (모듈 B 보정, 2-3시간)
4. **모듈 A 위성 GEE 시작** (1주+)
5. **VWorld 재시도** (외부 시스템 의존, 추후)
6. **팀원 collaborator 추가** (모듈 B/D 완성 후 결정 — 현재 단계)
7. **README v5 업데이트** — 누락된 cost_function, schemas 추가

### 보완 (작은 추가)
- 충북 보은 지역 특화 운반비 (KOFPI 전국 평균 → 지역)
- 보육·간벌 재투입 비용 모델
- 표준품셈 26개 할증 중 작업시기·이동거리·집단화 추가