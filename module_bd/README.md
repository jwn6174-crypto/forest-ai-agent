...
**작은 표 3개 (해송, 삼나무, 이태리포플러) 의 부분 데이터 손실 및 잘못된 정렬:**

| 수종 | 페이지 | 채운 값 | 예상 | 신뢰도 |
|---|---|---|---|---|
| 해송 수피포함 | p.30 | 123/216 | 일부 잘못 정렬 | DRAFT |
| 삼나무 수피포함 | p.56 | 134/216 | 일부 잘못 정렬 | DRAFT |
| 이태리포플러 수피포함 | p.102 | 156/216 | 일부 잘못 정렬 | DRAFT |

**원인:**  
작은 표도 2 PDF 페이지로 분할되어 있음 (예: 해송 = p.30 + p.31). 현재 첫 페이지만 처리해서 큰 DBH (22cm+) 값 누락. 게다가 pdfplumber 텍스트 추출 시 페이지 경계가 모호해서 일부 값이 잘못된 위치에 매핑됨.

**검증 사례 (해송):**  
DBH 6cm × 수고 22m = 0.3652 m³ 라고 추출되었으나, 작은 DBH × 큰 수고는 물리적으로 불가능한 조합. 페이지 31의 큰 DBH 값이 잘못 들어온 것으로 추정.

**영향 평가:**  
충북 보은 (파일럿 지역) 주력 수종은 강원지방소나무, 잣나무, 낙엽송. 작은 표 수종은 충북에서 거의 사용되지 않음. *공모전 시연 영향 작음*. 단, 학습 모델·DataFrame 사용 시 `품질 == "OK"` 필터링 필수.

**해결 방향 (추후):**
1. 페이지 30 + 31 (또는 56+57, 102+103) 결합 로직 작성
2. 또는 pdfplumber `extract_text()` 대신 `extract_words()` + bbox 좌표로 페이지 정확히 분리
3. `growth_predict()` 함수에서 작은 표 수종 호출 시 사용자에게 경고 메시지 반환
