...
class MarketSnapshot(BaseModel):
    ...
    timber_price_by_species: Optional[Dict[str, Dict[str, Optional[int]]]] = Field(
        None,
        description="7 수종 × 6 등급. 예: {'잣나무': {'1등급': 154700, ...}}"
    )
    
    # 우리 확장 — 출처 메타
    timber_price_meta: Optional[Dict[str, Any]] = Field(
        None,
        description="가격 데이터 출처·기간·법적 근거·단위 환산"
    )
    kau_meta: Optional[Dict[str, Any]] = Field(
        None,
        description="KAU vintage·실제 거래일·KOC 출처"
    )
...
class CostInput(BaseModel):
    ...
    action: Literal["clearcut", "thinning", "planting"] = Field(..., description="작업 종류")
...
class RotationRule(BaseModel):
    ...
    ownership: Literal["사유림", "국유림", "공유림"] = Field("사유림", description="소유 구분")
...
if __name__ == "__main__":
    ...
    print(f"   ✅ MarketSnapshot 확장: 잣나무 1등급={snap2.timber_price_by_species['잣나무']['1등급']}")
    ...
    print(f"   ✅ CostInput: {inp.action}, {inp.volume_m3}m³, {inp.distance_to_road_km}km")
    
    # 검증 5: GrowthForecast
    print("\n[검증 5] GrowthForecast from trajectory")
    fake_traj = [
        {"age": 30, "volume": 173.0, "dbh": 16.9, "height": 12.0, 
         "n_per_ha": 1261, "tmai_m3_per_ha_yr": 5.77, "method": "exact"},
        {"age": 50, "volume": 281.0, "dbh": 22.0, "height": 17.5, 
         "n_per_ha": 950, "tmai_m3_per_ha_yr": 5.62, "method": "exact"},
    ]
